...
class Ui_MainWindow(object):
    ...
    def delete(self):
        ...
        self.cursor.execute(f"SELECT * FROM detalle_articulo WHERE id_venta = {self.sale_id};") 
        condicion1 = self.cursor.fetchall()
        if len(condicion1) > 0:
            self.cursor.execute(f"DELETE FROM detalle_articulo WHERE id_venta = {self.sale_id};")
        self.cursor.execute(f"DELETE FROM venta WHERE id_venta = {self.sale_id};")
        self.mainWindow.close()
